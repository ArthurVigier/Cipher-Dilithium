...
@video_api.route('/generate_signature', methods=['POST'])
...
@video_api.route('/generate_signature_from_video', methods=['POST'])
def generate_signature_from_video_route():
    ...
    for video_file in video_files:
        ...
        # Use the video data as the message for generate_signature
        pq_signature, ecc_signature, pq_public_key, ecc_public_key = generate_signature(video_data)
...
@video_api.route('/download_file/<file_id>', methods=['GET'])
...
@video_api.route('/verify_signature', methods=['POST'])
def verify_signature_route():
    app.logger.info('Entered verify_signature_route')
    # Check if a file was provided
    if 'file' not in request.files or 'video' not in request.files:
        return jsonify({"error": "No file or video provided"}), 400

    file = request.files['file']
    video_file = request.files['video']

    app.logger.info('File and video received for signature verification')
